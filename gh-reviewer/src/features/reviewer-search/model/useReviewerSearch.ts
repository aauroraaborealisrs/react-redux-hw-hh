...
export function useReviewerSearch(
  ...
  const contributors = useAppSelector(
    (state) => state.github.contributorsByRepo[repoKey]?.contributors ?? [],
  );
    ...
    (candidates: GithubUser[]) => {
      ...
      intervalRef.current = window.setInterval(() => {
        ...
        if (ticks >= totalTicks) {
          ...
          const winnerIndex = visibleList.findIndex(
            (user) => user.login === winner.login,
          );
          const finalOffset = Math.min(
            Math.max(0, winnerIndex - CENTER_INDEX),
            maxOffset,
          );
